/**
 * 4kyu_Decode the Morse code, advanced
 * https://www.codewars.com/kata/decode-the-morse-code-advanced
  ...
  1. Function decodeBits(bits), that should find out the transmission rate of the message, correctly decode the message to dots ., dashes - and spaces (one between characters, three between words) and return those as a string.
 ...
 */
  ...
  var decodeBits = function(bits) {
    ...
    let result = bits
        .replace(new RegExp('0'.repeat(7 * rate), 'g'), '   ') // '0000000'
        .replace(new RegExp('0'.repeat(3 * rate), 'g'), ' ')   // '000'
        .replace(new RegExp('1'.repeat(3 * rate), 'g'), '-')   // '111'
        .replace(new RegExp('1'.repeat(1 * rate), 'g'), '.')   // '1'
        .replace(new RegExp('0'.repeat(1 * rate), 'g'), '')    // '0'
